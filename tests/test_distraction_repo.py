...
class TestDistractionRepo(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.session_id = session_repo.insert(Session(subject="Matematik"))

    def tearDown(self):
        restore_db(self._old_conn)

    def test_insert_returns_id(self):
        distraction_id = distraction_repo.insert(
    ...
    def test_insert_requires_valid_session_id(self):
        with self.assertRaises(sqlite3.IntegrityError):
            distraction_repo.insert(Distraction(session_id=9999, category="Telefon"))

    def test_get_by_session_returns_only_matching_session(self):
        other_session_id = session_repo.insert(Session(subject="Fizik"))
        distraction_repo.insert(Distraction(session_id=self.session_id, category="Telefon"))
        distraction_repo.insert(Distraction(session_id=other_session_id, category="Sosyal Medya"))

        result = distraction_repo.get_by_session(self.session_id)

        self.assertEqual(len(result), 1)
        self.assertEqual(result[0].category, "Telefon")

    def test_get_by_session_orders_by_occurred_at_ascending(self):
        distraction_repo.insert(Distraction(
    ...
    def test_get_all_orders_by_occurred_at_descending(self):
        ...
        result = distraction_repo.get_all()
    ...
    def test_note_defaults_to_empty_string(self):
        distraction_repo.insert(Distraction(session_id=self.session_id, category="Telefon", note=""))
