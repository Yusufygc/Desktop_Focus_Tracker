...
from app.core.repositories import session_repo, distraction_repo
...
class TestSessionRepo(unittest.TestCase):
    ...
    def test_insert_returns_id(self):
        session_id = session_repo.insert(Session(subject="Matematik"))
        self.assertEqual(session_id, 1)

    def test_get_by_id_returns_inserted_session(self):
        session_id = session_repo.insert(Session(subject="Fizik", notes="ilk not"))
    ...
    def test_get_by_id_returns_none_when_missing(self):
        self.assertIsNone(session_repo.get_by_id(999))

    def test_update_end_roundtrips_isoformat(self):
        session_id = session_repo.insert(Session(subject="Kimya"))
        ended_at = datetime(2026, 1, 1, 12, 30, 0)

        session_repo.update_end(session_id, ended_at, "bitti", 3)
    ...
    def test_update_info_changes_subject_and_notes(self):
        session_id = session_repo.insert(Session(subject="Tarih"))

        session_repo.update_info(session_id, "Coğrafya", "güncellendi")
    ...
    def test_get_all_orders_by_started_at_desc(self):
        ...
        session_repo.insert(s1)
        session_repo.insert(s2)
    ...
    def test_delete_removes_session(self):
        session_id = session_repo.insert(Session(subject="Silinecek"))

        session_repo.delete(session_id)

        self.assertIsNone(session_repo.get_by_id(session_id))

    def test_delete_cascades_to_distractions(self):
        session_id = session_repo.insert(Session(subject="Dikkat Dağınık"))
        distraction_repo.insert(Distraction(session_id=session_id, category="Telefon"))
        distraction_repo.insert(Distraction(session_id=session_id, category="Sosyal Medya"))

        session_repo.delete(session_id)

        self.assertEqual(distraction_repo.get_by_session(session_id), [])
